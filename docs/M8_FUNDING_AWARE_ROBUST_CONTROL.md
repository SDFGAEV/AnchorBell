...
- M1-M7 and the shared evidence accumulator are unchanged; each public event is sampled once.
- Unknown, stale or contradictory contract/market state fails closed.

## Mutable validation layer
...
Every variant uses identical events, anchors, rules, random tape and evidence evidence. M8-Full is compared with M7 on paired episodes and lockbox sessions.
...
The canonical lineage is M1 -> M2 -> M3 -> M4 -> M5 -> M6 -> M7 -> M8. Each policy_matrix
...
Validation references: Binance TradFi price-index notice; Binance USD-M funding-rate API; He, Manela, Ross and von Wachter, Fundamentals of Perpetual Futures; Huang, Lehalle and Rosenbaum, queue-reactive LOB; Esfahani and Kuhn, Wasserstein DRO; Rockafellar and Uryasev, CVaR optimization.
